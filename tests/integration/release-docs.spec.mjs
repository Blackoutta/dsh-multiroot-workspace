...
assert.match(readme, /npm install --global @deepseek-ai\/dsh@0\.1\.0-rc\.6/)
assert.match(readme, /DSH_HOME=~\/dsh-test dsh plugin --profile web add dsh-multiroot-workspace@next/)
assert.match(readme, /DSH_HOME=~\/dsh-test dsh web --port 3080/)
assert.match(readme, /DSH_HOME=~\/dsh-test dsh plugin --profile web remove dsh-multiroot-workspace/)
